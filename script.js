...
document.querySelector('.iframe').addEventListener('click', function(event) {
    ...
    const pdfUrl = this.getAttribute('href'); // Get the URL of the PDF
    const iframe = document.querySelector('#pdfIframe');
    iframe.src = pdfUrl; // Set the iframe source to the PDF URL
    document.querySelector('#iframeContainer').classList.remove('hidden'); // Show the iframe
});